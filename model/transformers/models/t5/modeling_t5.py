...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        if config.use_struct_attn_bias:
            self.struct_bias_table = nn.Parameter(torch.zeros(self.n_heads, 5))
            if config.struct_bias_init == "fixed":
                for h in range(self.n_heads):
                    self.struct_bias_table.data[h] = torch.tensor([1.0, 0.95, 0.81, 0.63, 0.0])
            elif config.struct_bias_init == "default":
                nn.init.normal_(self.struct_bias_table, mean=0.0, std=0.02)
    ...
    def forward(
        ...
        # 添加structural attention bias
        if dep_distances is not None:
            struct_attn_bias = self.struct_bias_table[:, dep_distances].transpose(0, 1).unsqueeze(2)
            scores = scores + struct_attn_bias
